...
import { nemavaliVivaranIndex } from '../../constants/nemavaliVivaranIndex';
...
export default function nemavaliVivaranIndexScreen() {
  const router = useRouter();

  // Flatten data with indentation for subsections
  const flattenedIndex = [];
  nemavaliVivaranIndex.forEach(item => {
  ...
  const handlePressItem = (item) => {
    router.push({ pathname: `/nemavaliVivaran/${item.id}`, params: { sectionTitle: item.title } });
  };
  ...
  return (
    <SafeAreaView style={styles.screenContainer}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Nemavali Vivaran</Text>
      </View>
      <FlatList
        data={flattenedIndex}
